...
# Store the movie name in variable named top_2_movie
  ...
  bollywood_sort <- bollywood[order(-bollywood$Tcollection), ]
    
  top_2_movie <- bollywood$Movie [2][1]
  top_2_movie
...
# Store the names of 4 movies in same sequence in movie_result vector
	...
	bollywood_ocollection <- bollywood$Movie[which.max(bollywood$Ocollection)]
	bollywood_ocollection
	bollywood_wcollection <- bollywood$Movie[which.max(bollywood$Wcollection)]
	bollywood_wcollection
	bollywood_fwcollection <- bollywood$Movie[which.max(bollywood$Fwcollection)]
	bollywood_fwcollection
	
	movie_result <- c (bollywood_ocollection,bollywood_wcollection,bollywood_fwcollection,top_movie)
	movie_result
